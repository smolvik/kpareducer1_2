...
extern unsigned long _ram2_begin;
extern unsigned long _ram2_end;
extern unsigned long _ram2_flash;
...
void handler_reset(void)
{
	...
	*(destination++) = *(source++);
	...
	/* Copying data from Flash to RAM2 */
	source = &_ram2_flash;
	for (destination = &_ram2_begin; destination < &_ram2_end;)
	{
	*(destination++) = *(source++);
	}	
