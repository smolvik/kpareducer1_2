...
#define CRC_POLY_INIT 0xFFFFFFFF 
...
uint32_t crc32(uint8_t *buf, uint32_t sz)
{
	int i;
	uint32_t crc;
	...
	for(i = 0; i < sz; i++)
	{
		crc = ((crc >> 8)&0x00ffffff) ^	crc_table[0xff&(crc^buf[i])];
	}
...
const unsigned char crc_table_low[256] = {
...
const unsigned char crc_table_high[256] = {
